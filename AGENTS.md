...
### Rule 7: Git Recovery & Commit Discipline (CRITICAL)
- **EVERY change** must be committed and pushed — no exceptions, no matter how small
- **Never delete files** without first committing them to git
- **Before any file operation**: `git add -A && git commit -m "description"` to snapshot current state
- **Before any destructive command** (rm, mv, overwrite): commit first, then act
- **Post-commit hook** auto-pushes to GitHub — every commit is immediately recoverable
- **File recovery**: If a file is deleted, recover with `git checkout HEAD -- <path>` or `git log --all --full-history -- <path>` to find the last commit containing it
- **Working directory**: Frontend MUST be at `/var/www/ceche/` (persistent), NOT `/tmp/` (volatile)
- **Never work in `/tmp`** for any project files — it gets cleared on disk cleanup
